...
class CwWfTest(BaseWithDb):
    ...
    def test_status_cmd(self, server_p):
        ...
        try:
            ...
            print(result.output)
            raise
        expected_output = f"""Failed to get response from server at __URL__/api/workflows/v1/WF_ID/status
"""
        self.assertEqual(result.output, expected_output)
        server_p.assert_called()

        server.configure_mock(**{"url.return_value": "__URL__", "is_running.return_value": True, "query.return_value": Mock(ok=True, content='{"status":"Succeeded","id":"WF_ID"}'.encode())})
